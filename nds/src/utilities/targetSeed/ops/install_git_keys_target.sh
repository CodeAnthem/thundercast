...
# Description:   Copy deploy keys, known_hosts, and tcast host CLI under /mnt as /var/lib/tcast
# ==================================================================================================

# Description: Absolute path under repo-root tcast/ (sibling of nds/).
# Arguments:
# - rel: <String> Path relative to tcast/ (e.g. bin/tcast)
_nds_tc_src() {
    printf '%s/../../tcast/%s\n' "${SCRIPT_DIR}" "$1"
}
...
# Description: Seed tcast host CLI onto the target as /var/lib/tcast (all users via PATH).
# Arguments:
# - mount_root: <String> Target mount (default /mnt)
_nds_git_install_nds_helpers_to_target() {
    ...
    [[ -d "${src_root}/bin" && -d "${src_root}/lib" ]] || {
        warn "tcast source missing: ${src_root}"
        return 0
    }
