...
# nds_git_env_pullTo (SSH identity / wizard via store bridge).
# Arguments:
...
nds_cast_clone() {
    ...
    if _nds_cast_url_is_http "$repo_url" && _nds_cast_https_anonymous_ok "$repo_url"; then
        ...
        pull_url="$repo_url"
        if _nds_cast_url_is_http "$repo_url" && declare -f git_url_toSsh &>/dev/null; then
            pull_url="${ git_url_toSsh "$repo_url"; }" || pull_url="$repo_url"
        fi
        err="${ nds_git_env_pullTo "$pull_url" "$dest" 1 2>&1; }" || rc=$?
    fi
    if [[ "$rc" -ne 0 ]]; then
...
# Skips addFleetHost and toolkit — those are built-in NDS actions, not catalog scripts.
# Arguments:
...
# addFleetHost and toolkit are built-in (NDS_ACTION=addFleetHost / NDS_ACTION=toolkit).
# Arguments:
# - cast_root: <String> Catalog checkout
# Returns:
...
nds_cast_require_user_actions() {
    ...
    if [[ -z "$actions" ]]; then
        error "Catalog has no user actions — addFleetHost and toolkit are built-in NDS actions (NDS_ACTION=addFleetHost or NDS_ACTION=toolkit)"
        return 1
    fi
    printf '%s\n' "$actions"
    return 0
