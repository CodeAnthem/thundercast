...
_nds_test_import_action() {
    local name="$1"
    local setup="${NDS_ACTION_DATA[${name}_path]:-}"
    if [[ -n "$setup" ]]; then
        setup="${setup}setup.sh"
    else
        setup="${SCRIPT_DIR}/actions/${name}/setup.sh"
    fi
...
suite_actions() {
    ...
    unset NDS_ACTION_DATA
    declare -gA NDS_ACTION_DATA=()
    nds_app_actionManager_logic_discover "${SCRIPT_DIR}/actions" || true
    ...
    _nds_test_import_action addFleetHost
    ...
    if grep -q 'Do not use a repository you do not trust' \
        "${SCRIPT_DIR}/wizard/install/ui/install_flake_cast.sh" \
    ...
    if ! grep -q 'nds_cfg_set CAST_ACTION "addFleetHost"' \
            "${SCRIPT_DIR}/wizard/install/ui/install_flake_cast.sh" \
