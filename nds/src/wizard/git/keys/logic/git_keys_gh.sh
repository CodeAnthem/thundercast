#!/usr/bin/env bash
# ==================================================================================================
# NDS - Git ↔ GitHub orchestration (uses utilities/git + wizard warm)
# ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# Date:          Created: 2026-08-05 | Modified: 2026-09-02
# Description:   Git owns key paths / titles; collision UI in git/keys/ui; GH API in utilities/git
# ==================================================================================================
...
nds_git_register_deploy_key() {
    ...
    while true; do
        ...
        git_gh_register_deploy_key "$pub_file" "$owner" "$repo" "$title" "$collision" "$read_only"
        rc=$?
        if [[ "$rc" -eq "${GIT_GH_RC_TITLE_COLLISION:-41}" ]]; then
...
nds_git_register_account_key() {
    ...
    while true; do
        collision="${NDS_GH_KEY_TITLE_COLLISION:-${NDS_GIT_SSH_KEY_TITLE_COLLISION:-}}"
        git_gh_register_account_key "$pub_file" "$title" "$collision"
        rc=$?
        if [[ "$rc" -eq "${GIT_GH_RC_TITLE_COLLISION:-41}" ]]; then
            nds_git_ui_ask_gh_title_collision \
...
_nds_git_lock_git_urls() {
    ...
    tmpdir="$(mktemp -d)"
    if ! git_gh_repo_file_content "$owner" "$repo" "flake.lock" >"${tmpdir}/flake.lock" 2>/dev/null; then
        rm -rf "$tmpdir"
        return 0
    fi
