...
nds_app_actionManager_logic_selectFromEnv() {
...
nds_app_actionManager_logic_select() {
    if nds_app_actionManager_logic_selectFromEnv; then
        return 0
    fi
...
_nds_app_actionManager_configurePresets() {
...
nds_app_actionManager_logic_callFeature() {
    local fn="$1"
    shift
    local -A cfg=()
    local mode pair
...
nds_app_actionManager_logic_execute() {
    local action_name="$1"
    ...
    _nds_app_actionManager_configurePresets || return 1

    nds_app_actionManager_ui_runPreview || rc=$?
    [[ "$rc" -ne 0 ]] && return "$rc"
...
nds_app_actionManager_logic_main() {
    local rc=0
    while true; do
        NDS_CURRENT_ACTION=""
        nds_app_actionManager_logic_select || return 1
        rc=0
        nds_app_actionManager_logic_execute "$NDS_CURRENT_ACTION" || rc=$?
