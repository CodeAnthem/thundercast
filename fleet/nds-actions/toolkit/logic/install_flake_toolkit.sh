...
nds_toolkit_generate_operator() {
    ...
    nds_requireUtility age || return 1
    if [[ ! -f "${dest}/operator_age.txt" ]]; then
        if ! age_keygen -o "${dest}/operator_age.txt" \
    ...
    pub_age="${ age_keygen -y "${dest}/operator_age.txt" 2>/dev/null; }" || {
...
# Description: Seed /var/lib/nds-toolkit from the ThunderCast checkout (fleet/toolkit).
...
nds_toolkit_seed_scripts_to_target() {
    ...
    local repo src_git toolkit_rel="fleet/toolkit"

    if [[ -x "${dest}/current/toolkit.sh" ]]; then
        info "fleet toolkit already seeded at ${dest}/current"
        return 0
    fi
    mkdir -p "$dest" || return 1
    rm -rf "${dest}/src"

    # Seed from this ThunderCast checkout (fleet/toolkit), not CAST_REPO_URL (user catalogs).
    repo="${NDS_THUNDERCAST_REPO_URL:-https://github.com/CodeAnthem/thundercast.git}"

    src_git="${NDS_CAST_PROBE_DIR:-}"
    if [[ -f "${src_git}/${toolkit_rel}/toolkit.sh" && -d "${src_git}/.git" ]]; then
        if ! git clone --no-hardlinks "$src_git" "${dest}/src" 2>/dev/null; then
            git clone "$src_git" "${dest}/src" || {
                error "Could not clone fleet toolkit from catalog checkout"
                return 1
            }
        fi
    else
        git clone --depth 1 "$repo" "${dest}/src" || {
            error "Could not clone fleet toolkit from ${repo}"
            return 1
        }
    fi

    if [[ ! -f "${dest}/src/${toolkit_rel}/toolkit.sh" ]]; then
        error "Checkout has no ${toolkit_rel}/toolkit.sh"
        return 1
    fi
    ...
    chmod +x "${dest}/src/${toolkit_rel}/toolkit.sh" || true
    if [[ -f "${dest}/src/${toolkit_rel}/tcast-sops.sh" ]]; then
        chmod +x "${dest}/src/${toolkit_rel}/tcast-sops.sh" || true
    fi
    ln -sfn "src/${toolkit_rel}" "${dest}/current"
    # Host CLI / tcast-sops come from nixosModules.tcast + toolkit (systemPackages).
    info "Seeded fleet toolkit at ${dest}/current"
    return 0
}
