# fleet/exampleRepo — copy this to your private leaf

NDS clones **your** private git remote as `FLAKE_REPO_URL`, not this folder in-tree. Copy `fleet/exampleRepo/` to a new private repository **before** the first toolkit ISO.

- `FLAKE_REPO_URL` — your private leaf (write access on the ISO)
- `NDS_ACTION=toolkit` / `NDS_ACTION=addFleetHost` — built-in NDS actions, not catalog scripts
...
3. ISO → **addFleetHost** (`NDS_ACTION=addFleetHost`) → **manager** → first boot `swarm init`.
...
5. **addFleetHost** gateway / workers (join from sops). Enroll each machine from the toolkit menu.
