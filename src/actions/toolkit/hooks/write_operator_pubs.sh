...
_nds_toolkit_hook_write_operator_pubs() {
    ...
    mkdir -p "${flake_root}/.toolkit/operator" || return 1
    if [[ -f "${dest}/operator_age.pub" ]]; then
        cp "${dest}/operator_age.pub" "${flake_root}/.toolkit/operator/age.pub"
    fi
    if [[ -f "${dest}/toolkit_ssh.pub" ]]; then
        cp "${dest}/toolkit_ssh.pub" "${flake_root}/.toolkit/operator/ssh.pub"
    fi
}
